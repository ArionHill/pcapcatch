...
class instance_control():
    def __init__(self,device,start_str,xmlFilePath):
        ...
        self.xml_file_path = xmlFilePath
    ...
    def press_DF(self,uui_text="",uui_class="",uui_bounds=""):
        ...
        if self.count > 0 :
            if uui_class + ' : ' + uui_text in self.analysed_button_dict.keys():        ###该控件之前已经被触发过，返回
                if self.analysed_button_dict[uui_class + ' : ' + uui_text] == uui_bounds:
                    print '该控件已被触发'
                    return False
            ...
            try:
                ...
                self.analysed_button_dict[uui_class + ' : ' + uui_text] = uui_bounds
        ...
        time.sleep(3)
        self.device.dump(self.xml_file_path + str(self.count) + ".xml")
        ...
        for r in root.getElementsByTagName('node'):
            ...
            if ui_text != '' and (':' not in ui_text) and r.getAttribute('package') == self.app_pkg:
    ...
    def handle_ad(self):                                ###处理APP启动时的广告
        ...
        xmll = self.device.dump(self.xml_file_path + "start.xml")
        update_related_button = []
        other_related_button = []
        # with open(self.xml_file_path + "start.xml",'w+') as f:
        #     f.write(xmll)
        ...
        for r in root.getElementsByTagName('node'):
            if r.getAttribute('package') == self.app_pkg:
                ...
                if r.getAttribute('scrollable') == 'true':
                    ...
                    self.device.swipe(0.9*int(x2),0.9*int(y2),1.1*int(x1),1.1*int(y1))
                    # print 1.1*int(x1)
                    # print 1.1*int(y1)
                    # print 0.9*int(x2)
                    # print 0.9*int(y2)
                if '跳过' in r.getAttribute('text'):
                    self.device(resourceId=r.getAttribute('resource-id')).click()
                    self.handle_ad()
                if '忽略' in r.getAttribute('text'):
                    self.device(resourceId=r.getAttribute('resource-id')).click()
                    self.handle_ad()
                if '以后再说' in r.getAttribute('text'):
                    self.device(resourceId=r.getAttribute('resource-id')).click()
                    self.handle_ad()
                if '允许' in r.getAttribute('text'):
                    self.device(resourceId=r.getAttribute('resource-id')).click()
                    self.handle_ad()
...
@TimeoutExceptionYcao.timelimited(100)
...
if __name__ == '__main__':
    ...
    # tv.danmaku.bili/tv.adnmaku.bilibi.ui.splash.SplashActivity