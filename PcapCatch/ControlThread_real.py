...
class ControlThread(threading.Thread):
        ...
        while self.hasApk():
            try:
                ...
                while self.isInstallSuccess == False:
                    ...
                    #获取设备已经安装的APP包名
                    package_list_original = self.getInstalledPackages()
                    # 安装apk
                    self.installApk()
                # 记录模拟器当前状态为busy
                self.setBusy(1)
                ...
                self.closeApk()
                # 卸载apk
                self.uninstallApk(self.apk.pkg)
                #获取设备当前已安装了的APP包名
                package_list_current = self.getInstalledPackages()
                print package_list_current
                #对额外安装的apk进行卸载
                for plc in package_list_current:
                    if plc not in package_list_original:
                        packageName = plc.split('package:')[1]
                        self.uninstallApk(packageName)
                # 分析完毕
                ...
                if self.bugNum >= 2:
                    ...
                    try:
                        ...
                        self.log.error("move error : %s" % ex)
                        self.log.error("%s" % traceback.format_exc())
                    finally:
                        self.apk = None
                        self.bugNum = 0
    ...
    # 复制tcpdump到/data/local目录下
    def copyTcpdump(self):
        cmd = ['push','/home/hill/Documents/iMiss/tcpdump','/data/local/tcpdump']
        self.run_adb_cmd(cmd)

    # 设置屏幕休眠
    def setSleepTime(self):
        cmd = ['shell','settings','put','system','screen_off_timeout','1800000']
        self.run_adb_cmd(cmd)
    ...
    def preparAnalyzeAPK(self):
        ...
        p.communicate()
        time.sleep(5)
        cmd = ['adb','shell','ls','/data/local']
        p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        ret = p.communicate()[0]
        if 'tcpdump' not in ret:
            self.copyTcpdump()
            time.sleep(5)
        cmd = ['adb','shell','chmod','-R','777','/data/local/']
        p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        p.communicate()
        time.sleep(5)
        cmd = ['adb', 'shell', '/data/local/tcpdump', '-i', 'wlan0','-w', '/sdcard/netdata.pcap', '-s',str(0), '-vvv', 'not', 'host', '10.0.2.2']
